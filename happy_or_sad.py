...
if st.session_state["page"] == "home":
    ...
    with col1:
        if st.button("👤 Single-player"):
            ...
            st.session_state["page"] = "single_intro"
            st.rerun()

    with col2:
        if st.button("👥 Two-players"):
            st.session_state["mode"] = "double"
            st.session_state["page"] = "double_intro"
            st.rerun()

# Introduction page
elif st.session_state["page"] == "single_intro" or st.session_state["page"] == "double_intro":

    # Introduction to single player mode
    if st.session_state["mode"] == "single":
        st.title("Introduction to single player mode")
        st.write("Imitate the expressions randomly given by the system and challenge the highest similarity!")
    
    # Introduction to two players mode
    elif st.session_state["mode"] == "double":
        st.title("Introduction to two players mode")
        st.write("The two players take turns imitating the target's expression to see who has the better match!")
...
# Game page
    ...
    col1, col2 = st.columns(2)
    with col1:
        restart = st.button("Again")
    with col2:
        exit_game = st.button("End")

    # If the button is clicked, prioritize the execution of the page jump logic
    # Again
    if restart:
        st.session_state["reference_image"]
        st.session_state["page"] = "game"
    ...
    # End
        ...
        # If is in single player mode
        if st.session_state["mode"] == "single":
            st.session_state["page"] = "single_intro"
            st.rerun() 

        # If is in two player mode
        else:
            st.session_state["page"] = "double_intro"
            st.rerun()
    ...
    def countdown_with_live_monitoring(video_placeholder, score_placeholder):
        ...
        with mp_face_mesh.FaceMesh(
            ...
            while time.time() - start_time < 5:
                ...
                if capture_success:
                    # Ask Chatgpt to mirror the camera
                    frame = cv2.flip(frame, 1)
                    video_frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
                    video_placeholder.image(video_frame_rgb, 
                                            caption="Camera", 
            ...
            # Two-player mode 
                ...
                if faces and len(faces) >= 1:
                    ...
                    if landmarks_1 is not None:
                        ...
                        player_score = similarity_score_1
    ...
    # Compare scores and show winners
    # Single player mode
    if st.session_state["mode"] == "single":
    ...
    # Two-player mode
        ...
        player1_score = countdown_with_live_monitoring(video_placeholder_1, score_placeholder_1)

        # player 2
        col2.subheader("player 2")
        video_placeholder_2 = col2.empty()
        score_placeholder_2 = col2.empty()
        player2_score = countdown_with_live_monitoring(video_placeholder_2, score_placeholder_2)
